...
-- transcation snapshot fact_statement_charge table: individual charge per line 
...
-- cumulative fact_statement_summary_table: one record per statement (monthly summary)
...
SELECT 
  ...
  COUNT(*) AS total_days FROM dim_date;
  
-- part d. dimension ETL procedures
delimiter //

DROP PROCEDURE IF EXISTS etl_load_dim_charge_type //
...
DROP PROCEDURE IF EXISTS etl_load_dim_landlord //
...
DROP PROCEDURE IF EXISTS etl_load_dim_property //
CREATE PROCEDURE etl_load_dim_property()
BEGIN
    DECLARE v_rows_expired  INT DEFAULT 0;
    DECLARE v_rows_inserted INT DEFAULT 0;


    -- Step 1: expire edition（SCD Type 2）
    UPDATE dim_property dp
    JOIN property p       ON dp.property_id = p.property_id
    ...
    LEFT JOIN agency_agent a 
           ON pa.agent_id = a.agent_id
    SET 
        dp.is_current  = FALSE,
        dp.expiry_date = DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY)
    WHERE dp.is_current = TRUE
      AND (
              dp.landlord_name           <> l.landlord_name
           OR COALESCE(dp.property_alias,'')      <> COALESCE(p.property_alias,'')
           OR COALESCE(dp.property_type,'')       <> COALESCE(pt.property_type_name,'')
           OR COALESCE(dp.property_address,'')    <> COALESCE(p.property_address,'')
           OR COALESCE(dp.property_city,'')       <> COALESCE(p.property_city,'')
           OR COALESCE(dp.property_postcode,'')   <> COALESCE(p.property_postcode,'')
           OR COALESCE(dp.property_bedrooms,0)    <> COALESCE(p.property_bedrooms,0)
           OR COALESCE(dp.current_agent_name,'')  <> COALESCE(a.agent_name,'')
           OR COALESCE(dp.current_agent_role,'')  <> COALESCE(a.agent_role,'')
      );

    SET v_rows_expired = ROW_COUNT();
    
    -- Step 2: insert new current edition
    --    include：
    --    1）new property
    --    2）just expire property（now don't have is_current = true）
    ...
    -- Step 3:return  SCD2 change
    SELECT 
        'dim_property loaded (SCD2)' AS result,
        v_rows_expired  AS rows_expired_scd2,
